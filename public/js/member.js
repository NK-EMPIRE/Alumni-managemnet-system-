...
window.submitEmailCampaignLaunch = function () {
    ...
    fetch('/api/v1/email-campaigns', {
        ...
        body: JSON.stringify({
            campaign_name: 'Member Email Update Campaign ' + new Date().toLocaleDateString(),
            testMode: !!(document.getElementById('campaignTestMode') && document.getElementById('campaignTestMode').checked)
        })
    }).then(function (r) { return r.json(); }).then(function (res) {
        if (res && res.success && res.data) {
