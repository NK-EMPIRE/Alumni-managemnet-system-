...
CREATE TABLE Alumni (
    ...
    faculty_assigned VARCHAR(150),
    resolved_faculty_user_id INT,
...
CREATE TABLE AlumniAssignments (
    ...
    CONSTRAINT FK_AA_AssignedBy FOREIGN KEY (assigned_by) REFERENCES Users(user_id),
    CONSTRAINT UQ_AlumniAssignments_Alumni UNIQUE (alumni_id)
);

-- ─── FacultyAliases ───────────────────────────────────────────
CREATE TABLE FacultyAliases (
    alias_id INT IDENTITY(1,1) PRIMARY KEY,
    faculty_id INT NOT NULL,
    alias_name VARCHAR(255) NOT NULL,
    CONSTRAINT FK_FacultyAliases_Users FOREIGN KEY (faculty_id) REFERENCES Users(user_id),
    CONSTRAINT UQ_FacultyAliases_Faculty_Alias UNIQUE (faculty_id, alias_name)
);
